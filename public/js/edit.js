...
// Socket Stuff //
  ...
  const getProject = function () {
    $.ajax({ url: `/get/${projectId}`, method: "GET" }).then(function (dbLoad) {
      let projectBody = $.parseHTML(dbLoad.projectContent);
      const projectItem = (
        `
      <p>${dbLoad.html}"</p>
      <p>${dbLoad.css}"</p>
      <p>${dbLoad.name}</p>
      <p>${dbLoad._id}</p>
      <div class="docArea" contenteditable="true" id ="bodyProject" value=""></div>
    `
      );

      $('#projectEdit').html(projectItem);
      $('#bodyProject').html(projectBody);
      // $('#saveNew').on('click', createProject);
